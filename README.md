...
- [ ] setopt incappendhistory    # Append commands to history file as each command is entered
- [ ] setopt histexpiredupsfirst # Remove oldest duplicate commands first
- [ ] setopt histignoredups      # Do not append consecutive duplicate commands to history list
- [ ] setopt histreduceblanks    # Trim white space from commands before adding to history list
...
- [ ] setopt pushdignoredups # Do not push multiple copies of same directory onto directory stack
...
- [ ] git graph and commit graph write on fetch
- [ ] git untracked cache
- [ ] git fsmonitor
- [ ] git clone filter blob, filter tree, and depth
- [ ] git sparse-checkout

- [ ] git branch with column
- [ ] git pull ff only
- [ ] git commit with fixup, etc. and rebase autosquash
- [ ] git restore, switch, reset, revert, worktrees aliases
- [ ] git rebase auto update refs
